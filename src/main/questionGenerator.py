...
def genBinQuestions(sent):
	q_strs = []
	q_strs.append(genBeQ(sent))
	q_strs.append(genBinOtherQ(sent))
	q_strs.append(genBePassiveQ(sent))
	return q_strs
...
def genWhQuestions(sent):
	q_str = []
	q_str.append(genWhatIs(sent))
	q_str.append(genWhatDoes(sent))
	q_str.append(genWho(sent))
	q_str.append(genWhere(sent))
	q_str.append(genWhen(sent))
	return q_str
...
def sort_by_score(questions_list, num_questions):
	...
	for q in questions_list:
		if len(q) > 3:
			list.append(q)
	sentences = sorted(list, key = lambda  x : q_score(x))
...
TARGET_LEN = 10
def q_score(q):
	total_score = abs(q.count(' ') - TARGET_LEN)
	return total_score
...
# 	#printInfo(sent)
# 	q_str = []
# 	for q in genBinQuestions(sent):
# 		q_str.append(q)
# 	for q in genWhQuestions(sent):
# 		q_str.append(q)
# 	return q_str
#
